...
lr = 0.001
svm = dualSVM.DualSVM(train_df, lr, 0, 1)
pred_train = svm.predict(train_df.drop(columns=['label']))
print(sum(abs(pred_train - train_df['label']))/(2*len(train_df)))

pred = svm.predict(test_df.drop(columns=['ID']))
...
predictions.to_csv("prediction_svm.csv", index=False)
